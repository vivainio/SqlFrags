...
type Tests() =
    ...
    static member TestOrm() =
        ...
        |> rendersTo "select\n  Salary, Name\nfrom employee"
        [ Emp.SelectAll ] |> rendersTo "select * from employee"
        [ Where [ Emp?ID ===^ "@ID" ] ] |> rendersTo "where employee.ID=@ID"
        [ // === (where condition with quoting)
          Where [ Emp?ID === "jorma" ] ]
        |> rendersTo "where employee.ID='jorma'"
        [ Emp.Delete ] |> rendersTo "delete from employee"
    ...
    static member TestNesting() =
        [ Many [ Many [ Raw "Inner" ] ] ] |> rendersTo "Inner"
        [ Nest [ Raw "Inner" ] ]
        |> rendersTo "(\n    Inner\n)"
        let onetwolj =
            [ LineJoiner(LineJoiners.ParensAndCommas,
                         [ Raw "One"
                           Raw "Two" ]) ]
        onetwolj
        |> rendersTo "(\nOne,\nTwo\n)"
    ...
    static member TestCreates() =
        let Emp = Table "Employee"
        [ Alter.CreateTable Emp [ "Name", VarChar 50
                                  "Salary", Number(10, 2)
    ...
    static member TestSyntax() =
        let r =
            [ RawSyntax [ SqlSyntax.Any, "hello sql"
    ...
    static member TestConds() =
        ...
        let conds =
            Conds.And [ e?a.Equals "12"
                        e?b.Equals "13"
                        Conds.Or [ e?c.Equals "14"
                                   e?d.Equals "15" ]
                        Conds.And [ e?e.Equals "16"
                                    e?f.Equals "17" ] ]
        [ conds.AsFrag ]
        |> rendersTo
               "(\n    emp.a=12\n    and\n    emp.b=13\n    and\n    (\n        emp.c=14\n        or\n        emp.d=15\n    )\n    and\n    (\n        emp.e=16\n        and\n        emp.f=17\n    )\n)"
        [ Where [ conds ] ]
        |> rendersTo
               "where (emp.a=12 and emp.b=13 and (emp.c=14 or emp.d=15) and (emp.e=16 and emp.f=17))"
        [ Conds.Where <| Conds.And [ e?a.Equals "10"
                                     e?b.Equals "11" ] ]
        |> rendersTo
               "where\n    (\n        emp.a=10\n        and\n        emp.b=11\n    )"
        [ Conds.Where <| Conds.And [ Conds.Gt e?a "10"
                                     Conds.Lt e?a "12"
                                     Conds.Gte e?b "20"
                                     Conds.Lte e?b "40" ] ]
        |> rendersTo
               "where\n    (\n        emp.a > 10\n        and\n        emp.a < 12\n        and\n        emp.b >= 20\n        and\n        emp.b <= 40\n    )"
        [ Conds.Where <| Conds.Exists [ Raw "select 1" ] ]
        |> rendersTo "where\n    exists\n    (\n        select 1\n    )"
    ...
    static member PlSql() =
        let q =
            [ Pl.Exec [ Raw "hello"
                        Raw "o'malley" ] ]
        q
        |> rendersToSyntax SqlSyntax.Any
               "execute sp_executesql\n'    hello,\n    o''malley'"
        q
        |> rendersToSyntax SqlSyntax.Ora
               "execute immediate\n'    hello,\n    o''malley'"
        ...
        |> rendersTo "ab\ncd;"
        [ // if then
          Pl.IfThen "a > 10" [ [ Raw "stuff in then" ] |> Pl.Stm
                               [ Raw "other line" ] |> Pl.Stm ] ]
        |> rendersTo
               "if a > 10\nthen\n    stuff in then;\n    other line;\nend if;"
        let vdef = [ VarDef("foo", VarChar 10, "'hi'") ]
                  ...
                  Pl.Begin
                      [ Pl.IfThen "not db_version_exists(new_version)"
                            [ Many frags
                              ...
                              Raw
                              <| sprintf
                                     "insert into VERSIONS values (new_version, '%s', CURRENT_TIMESTAMP)"
                                     category ] ] ]

        let upgradeSlug =
            createFrame "SOMEPRODUCT" "1212-2323" [ Emp.Insert [ "foo", "1" ] ]
        [ upgradeSlug ]
        |> rendersToSyntax SqlSyntax.Ora
    ...
    static member TypedTest() =
        let tt = TestType()
        let emp = Table "employee"
        [ emp.Update <| Typed.AsList <@ tt.bar <- 12
                                        tt.foo <- "hello"
                                        tt.mybool <- false @> ]
        |> rendersTo
               "update employee\nset bar = 12, foo = 'hello', mybool = false"
        [ emp.Insert <| Typed.AsList <@ tt.bar <- 12
                                        tt.foo <- "huuhaa" @> ]
        |> rendersTo "insert into employee (bar,foo) values (12,'huuhaa')"
        let fortable = Typed.AsListForTable emp <@ tt.bar <- 9 @>
    ...
    static member TestFuncs() =
        let s =
            ...
            |> Funcs.IsNull "11"
            |> Funcs.Avg
            |> Funcs.Convert "tinyint"
            |> Funcs.Sum
            |> Funcs.Count
            |> Funcs.Replace "'a'" "'b'"
            |> Funcs.ReplaceQuoted "a" "b"
            |> Funcs.As "hippie"
        Assert.AreEqual
            (s,
             "replace(replace(count(sum(convert(tinyint,avg(isnull(Foo,11))))), 'a', 'b'), 'a', 'b') as hippie")

    [<Case>]
    static member TestTextUtil() =
        let emp = Table "employee"
        let wrapped =
            TextUtil.TextWrap "-" 10 (Array.replicate 10 "hey!")
            |> String.concat "&"
        Assert.AreEqual
            (wrapped, "-hey!hey!hey!&-hey!hey!hey!&-hey!hey!hey!&-hey!")
