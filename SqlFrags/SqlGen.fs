...
and ColRef =
    ...
    static member PrefixTable prefix (ColRef(Table(t), col)) =
        ColRef((Table(prefix + t)), col)

let inline (?) (this : Table) (colName : string) = this.Col colName
...
type Frag =
    ...
    | NestAs of string * Frag seq
     // (provide alias for nested seg, e.g. (select ID from Emp) MyIds
     | Raw of string
    | RawSyntax of (SqlSyntax * string) seq
     // select string to emit by syntax
...
and Cond =
    ...
    member x.AsFrag =
        ...
        | CondCompose(op, parts) ->
            parts
            |> Seq.map (fun p -> p.AsFrag)
            |> TextUtil.Interleave(Raw op)
            |> Nest
        | CondFrag(f) -> f
        | _ -> Raw x.Str
...
let ColumnsS(cols : string seq) =
    cols
    |> TextUtil.Colonize
    |> TextUtil.TextWrap "  " 75
    |> Seq.map Raw
    |> Many

// Columns that takes a list of colrefs
let Columns(cols : ColRef seq) =
    cols
    |> Seq.map (fun c -> c.Str)
    |> ColumnsS
...
module Pl =
    ...
    let Exec frags =
        Many [ RawSyntax [ SqlSyntax.Any, "execute sp_executesql"
                           SqlSyntax.Ora, "execute immediate" ]
               Indent [ LineJoiner(LineJoiners.SingleQuotes, frags) ] ]
...
let rec serializeFrag (syntax : SqlSyntax) frag =
    ...
    | SelectAs cols ->
        "select " + (cols
                     |> Seq.map
                            (fun (c, alias) -> sprintf "%s as %s" c.Str alias)
                     |> colonList)
    ...
    | JoinOn(other : ColRef, this : ColRef, (Table alias), joinKind : string) ->
        ...
        let joinType =
            if joinKind = "" then "inner"
            else joinKind

        sprintf "%s join %s %s on %s=%s" joinType otherTable alias this.Str
            aliasedOther.Str
    | Raw txt -> txt
    | RawSyntax(rules) ->
        rules
    ...
    | Many _
    | LineJoiner _
    | Indent _ -> failwith "Should never see subquery at serialization"
...
let serializeSql syntax frags =
    ...
    let rec emitFrags nestingLevel frags =
        ...
        let doIndent subfrags =
            (emitFrags (nestingLevel + 1) subfrags
             |> String.concat "\n")
...
module Alter =
    let Operation (operation : string) (col : ColRef) (typ : DDLType) =
        let (ColRef(t, c)) = col
        Pl.Stm [ Raw <| sprintf "alter table %s %s %s" t.Name operation c
                 TypeName typ ]

    let AddCol (col : ColRef) (typ : DDLType) = Operation "add" col typ
    ...
    let CreateIndex (tab : Table) (idxName : string) (cols : string seq) =
        Raw
        <| sprintf "create index %s on %s (%s);" idxName tab.Name
               (colonList cols)
    ...
    let CreateTable (Table t) (cols : DDLCol list) =
        Many
            [ Raw <| sprintf "create table %s" t
              ...
              Indent
                  [ LineJoiner
                        (LineJoiners.ParensAndCommas, (cols |> List.map ColDef)) ] ]
...
module Typed =
        ...
        seq {
            for p in parts do
                ...
                | (PropertySet(Some(k), pi, _, valHolder)) ->
                    ...
                    | ValueWithName(value, _, _) -> yield (pi.Name, value)
                    //printfn "value %s is %A" pi.Name value
                    | Value(value, _) -> yield (pi.Name, value)
                    | _ -> failwithf "Unsupported val pattern %A" valHolder
                | _ -> ()
        }
    ...
    let AsList(tree : Expr) =
        extractProps tree
        |> Seq.map (fun (a, b) -> a, objAsString b)
        |> List.ofSeq
...
type Table with
    ...
    member x.Select cols =
        Many [ Raw "select"
               ColumnsS cols
               From x ]

    member x.SelectC cols =
        Many [ Raw "select"
               Columns cols
               From x ]
    ...
    member x.Update(updlist : (string * string) seq) =
        let updates =
            ...
            |> String.concat ", "
        Many [ Raw(sprintf "update %s" x.Name)
...
module Funcs =
    ...
    let Replace needle replacement s =
        sprintf "replace(%s, %s, %s)" s needle replacement
    let ReplaceQuoted needle replacement s =
        sprintf "replace(%s, %s, %s)" s (sqlQuoted needle)
            (sqlQuoted replacement)
...
// glorious public api
module Frags =
    let Emit syntax frags = serializeSql syntax frags
